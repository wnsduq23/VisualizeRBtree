...
void ProfileBegin(const WCHAR* szName)
{
	...
	for (; i < PROFILE_CNT; i++)
	{
		if (PROFILE_RESULT[i].lFlag != 0 &&
			wcscmp(PROFILE_RESULT[i].szName, szName) == 0)//같은 tagname의 PRO_BEGIN을 또 호출했을 경우
		{
		...
		else if (PROFILE_RESULT[i].lFlag == 0) // 아예 없으면
		{
			_PROFILE_RESULT& pf = PROFILE_RESULT[i];
			pf.lFlag = 1;
...
void ProfileDataOutText(const WCHAR* szFileName)
{
	...
	{
		_PROFILE_RESULT& pf = PROFILE_RESULT[idx];
		pf.dTotalTime -= (pf.dMax[0] + pf.dMin[0]);
		memset(buffer, '\0', BUFFER_SIZE);
		sprintf_s(buffer, BUFFER_SIZE,
			"| %ls | %.4lfμs | %.4lfμs | %.4lfμs | %lld |\n",
			pf.szName,
			(pf.dTotalTime / (pf.iCall - MINMAX_CNT)) * NS_PER_SEC, // 양쪽 과도한 MIN MAX는 평균에서 제외
			pf.dMin[0] * NS_PER_SEC,
			pf.dMax[0] * NS_PER_SEC,
			pf.iCall);
